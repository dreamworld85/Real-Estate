  ...
  return (
    <div className="px-4 py-5 flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        <span className="text-xs font-bold text-ink tracking-wide">Pending Review ({pendingReports.length})</span>
        {loading ? (
          ...
          pendingReports.map((r) => (
            <div
              key={r.id}
              className="bg-white border border-rose-200/60 rounded-2xl p-4 flex flex-col gap-3 shadow-sm"
            >
              <div className="flex gap-3 items-start">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate mt-1 italic leading-relaxed">
                    Reason: "{r.reason}"
                  </p>
              ...
              <div className="flex gap-2 border-t border-charcoal/5 pt-3 mt-1">
                ...
                <button
                  ...
                  className="flex-[2] py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl flex items-center justify-center gap-1 text-[10px] font-bold transition-colors shadow-sm"
                >
      ...
      {resolvedReports.length > 0 && (
        <div className="flex flex-col gap-3 mt-2">
          <span className="text-xs font-bold text-slate tracking-wide">Resolved Reports ({resolvedReports.length})</span>
          <div className="flex flex-col gap-2">
            {resolvedReports.map((r) => (
              <div
                key={r.id}
                className="bg-white/80 border border-charcoal/5 p-3 rounded-2xl flex items-center justify-between opacity-70"
              >
